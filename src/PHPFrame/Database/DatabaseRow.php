<?php
/**
 * PHPFrame/Database/DatabaseRow.php
 * 
 * PHP version 5
 * 
 * @category  PHPFrame
 ...
 *       conditional statements. Mainly: if ($field instanceof PHPFrame_DatabaseField).
 *       
...
class PHPFrame_DatabaseRow
{
     ...
     * PHPFrame_DatabaseRow::load(), as it can take an IdObject instead of an 
     * id as an argument.
     * 
     ...
     * @access public
     * @return void
     * @see    PHPFrame_Database_IdObject, PHPFrame_DatabaseRowCollection
     * @since  1.0
     */
    ...
    public function toString($show_keys=true)
    {
        ...
        foreach ($this->_fields as $field) {
            if ($show_keys) {
                ...
                $str .= PHPFrame_String::fixLength($field->getValue(), 16)."\t";
            }
        }
    ...
    public function toArray()
    {
        ...
        foreach ($this->_fields as $key=>$value) {
            if ($value instanceof PHPFrame_DatabaseField) {
    ...
    public function get($field_name)
    {
        ...
        if (in_array($field_name, $field_names)) {
            if ($this->_fields[$field_name] instanceof PHPFrame_DatabaseField) {
    ...
    {
        foreach ($this->_fields as $field) {
            if ($field instanceof PHPFrame_DatabaseField
                && $field->isPrimaryKey()) {
            ...
            foreach ($array as $key=>$value) {
                if (!in_array($key, $exclude)) {
                    if (isset($this->_fields[$key]) 
                        && $this->_fields[$key] instanceof PHPFrame_DatabaseField
                    ) {
    ...
    private function _fetchFields()
    {
        ...
        foreach ($table_structure as $field_array) {
            $this->_fields[$field_array["Field"]] = new PHPFrame_DatabaseField($field_array);
        }
    }
    ...
    private function _insert()
    {
        ...
        foreach ($this->_fields as $field) {
            // Only take into account fields of type PHPFrame_DatabaseField
