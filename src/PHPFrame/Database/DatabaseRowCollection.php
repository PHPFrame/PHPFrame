<?php
/**
 * PHPFrame/Database/DatabaseRowCollection.php
 * 
 * PHP version 5
 * 
 * @category  PHPFrame
 ...
 * $rows = new PHPFrame_DatabaseRowCollection(array("select"=>"email",
 *                                                   "from"=>"#__users",
 *                                                   "where"=>array("id", "=", "62")
 *                                                   )
 *                                             );
 ...
 * 
 * // Create RowCollection object
 * $rows2 = new PHPFrame_DatabaseRowCollection($options);
 * // Load the selection
 * $rows2->load();
 * // Print results
 ...
 * <code>
 * // Create RowCollection object
 * $rows3 = new PHPFrame_DatabaseRowCollection();
 * // Make a selection
 * $rows3->select("*")
 *       ->from("#__users")
 *       ->where("id", "=", ":id")
 ...
 * @see PHPFrame_Mapper
 */
class PHPFrame_DatabaseRowCollection implements Iterator
{
    /**
        ...
        for ($i=0; $i<count($this->_rows); $i++) {
            ...
            if ($i == 0) {
                ...
                foreach ($fields as $key) {
                    $str .= PHPFrame_String::fixLength($key, 16)."\t";
                }
                $str .= "\n";
            }
     ...
     * 
     * @access public
     * @return PHPFrame_DatabaseRowCollection
     * @since  1.0
     */
    ...
    private function _fetchRows(PHPFrame_Database_IdObject $id_object, $exclude='')
    {
        ...
        if (!($stmt instanceof PDOStatement)) {
            ...
            while ($array = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $row = new PHPFrame_DatabaseRow($table_name, $this->_db);
