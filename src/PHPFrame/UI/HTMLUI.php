...
class PHPFrame_HTMLUI
{
     ...
     * 
     * <code>
     * $options[] = PHPFrame_HTMLUI::_('select.option', $row->id, $row->name );
    ...
    public static function _($str) 
    {
        ...
        if (is_callable( array( 'PHPFrame_HTMLUI', $function_name) )) {
            $args = func_get_args();
            array_shift( $args );
            return call_user_func_array( array( 'PHPFrame_HTMLUI', $function_name ), $args );
        } else {
            throw new RuntimeException('PHPFrame_HTMLUI::'.$function_name.' not supported.');
        }
